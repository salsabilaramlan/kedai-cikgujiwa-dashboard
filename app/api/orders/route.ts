...
async function ready(){await env.DB.prepare("CREATE TABLE IF NOT EXISTS sheet_status (order_key TEXT PRIMARY KEY, status TEXT NOT NULL, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)").run()}
function csv(text:string){const rows:string[][]=[];let row:string[]=[],cell="",quote=false;for(let i=0;i<text.length;i++){const c=text[i];if(c==='"'){if(quote&&text[i+1]==='"'){cell+='"';i++}else quote=!quote}else if(c===","&&!quote){row.push(cell);cell=""}else if((c==="\n"||c==="\r")&&!quote){if(c==="\r"&&text[i+1]==="\n")i++;row.push(cell);if(row.some(Boolean))rows.push(row);row=[];cell=""}else cell+=c}if(cell||row.length){row.push(cell);rows.push(row)}return rows}
function money(value=""){return Number(value.replace(/[^0-9.-]/g,""))||0}
type Item={key:string;sheet:string;row:number;customer:string;name:string;details:string;quantity:number;amount:number;payment:string;date:string;phone:string;status:string;base:string;background:string;letter:string};
function normalize(tab:string,rows:string[][]){let lastCustomer="";return rows.slice(1).map((r,i):Item|null=>{const row=i+2;let customer="",name="",details="",quantity=1,amount=0,payment="",date="",phone="",base="",background="",letter="";
 if(tab==="CLICKER"){lastCustomer=r[0]||lastCustomer;customer=lastCustomer||"Pelanggan";name=r[1];base=r[2]?.trim()||"";background=r[3]?.trim()||"";letter=r[4]?.trim()||"";details=[base,background,letter].filter(Boolean).join(" · ");amount=money(r[6]);payment=r[5]}
 else if(tab==="PHOTO"){customer=r[1];name=r[3]||"Photo";amount=money(r[2]);payment=r[4]}
 else if(tab==="MAGNET"){customer=r[0];name="Fridge Magnet";quantity=Number(r[1])||1;amount=money(r[2]);payment=r[3]}
 else if(tab==="NAMETAG"){customer=r[0];name=r[1]||"Nametag";amount=money(r[3]);payment=r[2]}
 else if(tab==="KOPI"){date=r[0];customer=r[1]||"Pelanggan";name="Kopi";quantity=Number(r[2])||1;amount=money(r[4]);payment=r[3]}
